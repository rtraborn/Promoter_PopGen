...
def cat_gff_h(gfffile, h_file, outputfolder):
    ...
    with open(gfffile) as gf:
        for line in gf:
            ...
            linesp = line.split()
            strand = linesp[6]
            center = int(round((int(linesp[3]) + int(linesp[4])) / 2))
            left = center-250
            right = center+250
...
if __name__ == '__main__':
    for i in range(1,23):
        outsubfolder = os.path.join(outputfolder, 'gene_m_' + str(i))
        if not os.path.isdir(outsubfolder):
            os.mkdir(outsubfolder)
        cat_gff_h(os.path.join(inputfolder, 'Homo_sapiens.GRCh38.90.chromosome.' + str(i) + '_sorted_.gff3'),
